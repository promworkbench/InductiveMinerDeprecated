...
public class EfficientTreeMetrics {
	...
	public static long getShortestTrace(EfficientTreeAb tree, int node) throws UnknownTreeNodeException {
	 ...
	 *         trace.
	 */
	public static BitSet canOnlyProduceTau(EfficientTreeAb tree) {
	...
	public static boolean canOnlyProduceTau(EfficientTreeAb tree, int node) throws UnknownTreeNodeException {
		if (tree.isActivity(node)) {
			return false;
		} else if (tree.isTau(node)) {
			return true;
	...
	public static BitSet canProduceTau(EfficientTreeAb tree) {
		BitSet result = new BitSet(tree.getMaxNumberOfNodes());
		for (int node = tree.getMaxNumberOfNodes() - 1; node >= 0; node--) {
			if (tree.isActivity(node)) {
				result.set(node, false);
	...
	public static boolean canProduceTau(EfficientTreeAb tree, int node) throws UnknownTreeNodeException {
		if (tree.isActivity(node)) {
			return false;
		} else if (tree.isTau(node)) {
			return true;
	 ...
	 */
	public static boolean canProduceSingleActivity(EfficientTreeAb tree, int node, int activity)
			throws UnknownTreeNodeException {
		if (tree.isTau(node)) {
			return false;
	 ...
	 * @throws UnknownTreeNodeException
	 */
	public static boolean traceLengthAtMostOne(EfficientTreeAb tree, int node) throws UnknownTreeNodeException {
		if (tree.isActivity(node)) {
			return true;
		} else if (tree.isTau(node)) {
			return true;
	 ...
	 * @throws UnknownTreeNodeException
	 */
	public static boolean onlyEmptyTrace(EfficientTreeAb tree, int node) throws UnknownTreeNodeException {
		if (tree.isActivity(node)) {
			return false;
		} else if (tree.isTau(node)) {
			return true;
